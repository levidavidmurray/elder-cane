...
namespace EC.Core {
    ...
    public class KCController : MonoBehaviour, ICharacterController
    {
        ...
        public float RollCapsuleHeight = 1f;
        ...
        public bool ShouldBeRolling => Time.time - _rollStartTime < RollDuration;
                ...
                case CharacterState.Default:
                    {
                        ...
                        {
                            if (ShouldBeRolling) {
                                float rollTime = Time.time - _rollStartTime;
                                float rollSpeed = RollSpeedCurve.Evaluate(rollTime / RollDuration) * RollSpeedPeak;
                                AddVelocity(transform.forward * rollSpeed);
                            }
                ...
                case CharacterState.Default:
                    {
                        ...
                        bool shouldUnroll = _isRolling && !ShouldBeRolling;

                        // Handle uncrouching
                        if (shouldUncrounch || shouldUnroll)
                        {
